    ...
    def forget_password(self):
        if self.cmb_quest.get()=="select" or self.txt_answer.get()=="" or self.txt_new_pass_.get()=="":
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from user where email=%s and question=%s and answer=%s",(self.txt_email.get(),self.cmb_quest.get(),self.txt_answer.get()))
                row=cur.fetchone()
                # print(row)
                if row==None:
                    messagebox.showerror("Error","Please Select Correct Security Question / Enter Answer",parent=self.root2)   
                else:
                    cur.execute("update user set password=%s where email=%s",(self.txt_new_pass_.get(),self.txt_email.get()))
                    con.commit()
                    con.close()
